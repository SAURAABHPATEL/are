...
const betWinGo = async (req, res) => {
    ...
    if (check >= 0) {
        ...
        today = ?,
        time = ?`;
        await connection.execute(sql, [id_product, userInfo.phone, userInfo.code, userInfo.invite, period, userInfo.level, total, amt, fee, 0, gameJoin, join, 0, checkTime, timeNow]);
