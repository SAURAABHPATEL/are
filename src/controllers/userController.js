...
const verifyCode = async (req, res) => {
    ...
    let timeEnd = (+new Date) + 100 * (60 * 2 + 0) + 500;
...
const aviator = async (req, res) => {
    let auth = req.cookies.auth;
    res.redirect(`https://domain.com/theninja/src/api/userapi.php?action=loginandregisterbyauth&token=${auth}`);
...
const promotion = async (req, res) => {
    ...
    const [f1s] = await connection.query('SELECT `phone`, `code`,`invite`, `time` FROM users WHERE `invite` = ? AND DATE_FORMAT(`today`,"%Y-%m-%d") = ?', [userInfo.code, yesterday]);
    ...
    let direct_register = f1s.length;
    let team_register = 0;

    let direct_deposit_number = [];
    let direct_deposit_amount = [];

    let team_deposit_number = [];
    let team_deposit_amount = [];

    for (let i = 0; i < f1s.length; i++) {
        const f1_code = f1s[i].code;
        const f1_phone = f1s[i].phone;
        const f1_time = f1s[i].time;
        let check_f1 = (timerJoin(f1_time) == timerJoin()) ? true : false;
        if (check_f1) f_all_today += 1;

        //get direct user deposit number
        const [d1s] = await connection.query('SELECT SUM(money) as deposit_num, SUM(status) as deposit_count FROM recharge WHERE `phone` = ? AND DATE_FORMAT(`created_at`,"%Y-%m-%d") = ?', [f1_phone, yesterday]);
        direct_deposit_number.push(d1s[0].deposit_count);
        direct_deposit_amount.push(d1s[0].deposit_num);


        // Total level-2 referrals today
        const [f2s] = await connection.query('SELECT `phone`, `code`,`invite`, `time` FROM users WHERE `invite` = ? AND DATE_FORMAT(`today`,"%Y-%m-%d") = ?', [f1_code, yesterday]);
        team_register += f2s.length;
        for (let i = 0; i < f2s.length; i++) {
            
            const f2_code = f2s[i].code;
            const f2_phone = f2s[i].phone;
            const f2_time = f2s[i].time;
            let check_f2 = (timerJoin(f2_time) == timerJoin()) ? true : false;
            if (check_f2) f_all_today += 1;


            //get team2 user deposit number
            const [d2s] = await connection.query('SELECT SUM(money) as deposit_num, SUM(status) as deposit_count FROM recharge WHERE `phone` = ? AND DATE_FORMAT(`created_at`,"%Y-%m-%d") = ?', [f2_phone, yesterday]);
            team_deposit_number.push(d2s[0].deposit_count);
            team_deposit_amount.push(d2s[0].deposit_num);

            // Total level-3 referrals today
            const [f3s] = await connection.query('SELECT `phone`, `code`,`invite`, `time` FROM users WHERE `invite` = ? AND DATE_FORMAT(`today`,"%Y-%m-%d") = ?', [f2_code, yesterday]);
            team_register += f3s.length;
            for (let i = 0; i < f3s.length; i++) {
                const f3_code = f3s[i].code;
                const f3_phone = f3s[i].phone;
                const f3_time = f3s[i].time;
                let check_f3 = (timerJoin(f3_time) == timerJoin()) ? true : false;
                if (check_f3) f_all_today += 1;

                //get team3 user deposit number
                const [d3s] = await connection.query('SELECT SUM(money) as deposit_num, SUM(status) as deposit_count FROM recharge WHERE `phone` = ? AND DATE_FORMAT(`created_at`,"%Y-%m-%d") = ?', [f3_phone, yesterday]);
                team_deposit_number.push(d3s[0].deposit_count);
                team_deposit_amount.push(d3s[0].deposit_num);
    ...
    // for (let i = 0; i < f1s.length; i++) {
    //     const f1_phone = f1s[i].phone;
    //     const [f2ss] = await connection.query('SELECT SUM(money) as deposit_num, SUM(status) as deposit_count FROM recharge WHERE `phone` = ? ', [f1_phone]);
    //     f2.push(f2ss[0].deposit_count);
    //     //if (f2s[0].deposit_num !== null ) f2_phone += f2s[0].deposit_num;
    //     f2_phone.push(f2ss[0].deposit_num);
    ...
    //             const ff2_phone = f3s[k].phone;
    //             const [f2ss2] = await connection.query('SELECT SUM(money) as sub_deposit_num, SUM(status) as sub_deposit_count FROM recharge WHERE `phone` = ? ', [ff2_phone]);
    //             sub_deposit_count.push(f2ss2[0].sub_deposit_count);
    //             //if (f2s[0].deposit_num !== null ) f2_phone += f2s[0].deposit_num;
    //             f3_phone.push(f2ss2[0].sub_deposit_num);
    //         }
    ...
    return res.status(200).json({
        ...
        invite: {
            ...
            direct_deposit_number:direct_deposit_number,
            direct_deposit_amount:direct_deposit_amount,
            team_deposit_number:team_deposit_number,
            team_deposit_amount:team_deposit_amount,
            f1_today: f1_today,
...
const listMyTeam = async (req, res) => {
    ...
    let userInfo = user[0];
    const [f1] = await connection.query('SELECT `id_user`, `phone`, `code`, `invite`,`roses_f`, `rank`, `name_user`,`status`,`total_money`, `today` FROM users WHERE `invite` = ? AND DATE_FORMAT(`today`,"%Y-%m-%d") = ? ORDER BY id DESC', [userInfo.code, mydate]);
    const [mem] = await connection.query('SELECT `id_user`, `phone`, `today` FROM users WHERE `invite` = ? AND DATE_FORMAT(`today`,"%Y-%m-%d") = ? ORDER BY id DESC LIMIT 100', [userInfo.code, mydate]);
    const [total_roses] = await connection.query('SELECT `f1`,`invite`, `code`,`phone`,`time` FROM roses WHERE `invite` = ? AND DATE_FORMAT(`today`,"%Y-%m-%d") = ? ORDER BY id DESC LIMIT 100', [userInfo.code, mydate]);
    ...
    async function fetchUserDataByCode(code, depth = 1) {
        ...
        const [userData] = await connection.query('SELECT `id_user`, `name_user`, `phone`, `code`, `invite`, `rank`, `total_money`, `time` FROM users WHERE `invite` = ? AND DATE_FORMAT(`today`,"%Y-%m-%d") = ?', [code, mydate]);
        if (userData.length > 0) {
            for (const user of userData) {
                ...
                const [depositCountData] = await connection.query('SELECT SUM(money) as deposit_count FROM recharge WHERE `phone` = ? AND DATE_FORMAT(`created_at`,"%Y-%m-%d") = ?', [user.phone, mydate]);
                const depositCount = depositCountData[0].deposit_count;

                const [depositNum] = await connection.query('SELECT COUNT(*) as deposit_num FROM recharge WHERE `phone` = ? AND DATE_FORMAT(`created_at`,"%Y-%m-%d") = ?', [user.phone, mydate]);
                const depositnum = depositNum[0].deposit_num;
                
                const [betCountData] = await connection.query('SELECT SUM(amount) as bet_amount FROM minutes_1 WHERE `phone` = ? AND DATE_FORMAT(`today`,"%Y-%m-%d") = ?', [user.phone, mydate]);
                const betAmount = betCountData[0].bet_amount;

                const [betNum] = await connection.query('SELECT COUNT(*) as bet_num FROM minutes_1 WHERE `phone` = ? AND DATE_FORMAT(`today`,"%Y-%m-%d") = ?', [user.phone, mydate]);
                const betnum = betNum[0].bet_num;

                const [firstRc] = await connection.query('SELECT * FROM recharge WHERE `phone` = ? AND DATE_FORMAT(`created_at`,"%Y-%m-%d") = ?', [user.phone, mydate]);
                var firstrecharge =0;
                var firstrechargeamount =0;
                if (firstRc.length == 1) {
                    firstrecharge = 1;
                    firstrechargeamount = firstRc[0].money;
                }
                
                const [rosesCountData] = await connection.query('SELECT SUM(f1) as comm_amount FROM roses WHERE `phone` = ? AND DATE_FORMAT(`today`,"%Y-%m-%d") = ?', [user.phone, mydate]);
                const commAmount = rosesCountData[0].comm_amount;
                ...
                const userObject = {
                    ...
                    first_recharge_amount: firstrechargeamount,
...
const recharge = async (req, res) => {
    ...
    if (recharge.length == 0) {
        ...
        const apiData = {
            ...
            redirect_url: `https://domain.com/wallet/verify/upi`,
            udf1: 'TIRANGA',
        };
...
const infoUserBank = async (req, res) => {
    ...
    let withdrawable_amt = userInfo.money - result;
